...
func genEncKeyAndSign(auth *rsa.PrivateKey) (
	...
	enc, _ = rsa.GenerateKey(rand.Reader, 2048)
...
func doubleRSAPFS() {
	bobAuth, _ := rsa.GenerateKey(rand.Reader, 2048)

	plaintxt := "Hello Bob this is Alice.0123456789abcdefghijklmnopqrstuvwxyz012"
...
func doubleRSAPFSMITM() {
	bobAuth, _ := rsa.GenerateKey(rand.Reader, 2048)
	oscarAuth, _ := rsa.GenerateKey(rand.Reader, 2048)
